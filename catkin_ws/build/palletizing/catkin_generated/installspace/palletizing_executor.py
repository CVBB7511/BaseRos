...
class PalletizingExecutor:
    ...
    def _mark_zone(self, req):
        ...
        rospy.loginfo("Zone '%s' marked: (%.2f, %.2f, %.2f)", req.zone_name, req.x, req.y, req.z)
    ...
    def _get_robot_position(self):
        """Get current robot base_link position in /map frame."""
        try:
            ...
            return self.source_table_x, self.source_table_y
    ...
    def _compute_collision_risk(self, objects, sorted_indices):
        ...
        Returns (risk, blocked, y_nbrs, arm_blocked, z_top, edge_x) per index.
        """
        n = len(objects.name)
        # Gather object data
        half = []     # half-size per object
        cx = []       # center X
        cy = []       # center Y
        cz = []       # center Z
        edge_x = []   # closest edge X (xMin)
        z_top = []    # top Z
        z_bot = []    # bottom Z
        for i in range(n):
            ...
            zt = objects.z[i] if i < len(objects.z) else 0.0
            z_top.append(zt)
            cz.append(zt - hsize)
            z_bot.append(zt - 2.0 * hsize)
        ...
        arm_blocked = [False] * n  # arm body would hit taller rear object
        y_nbrs = [0] * n
            ...
            for j in range(n):
                ...
                if edge_x[j] > edge_x[i]:  # j is behind i
                    if z_overlap_ij and y_overlap:
                        # Grasping front i: arm body would hit rear j
                        arm_blocked[i] = True
                        intrusion = (z_top[j] - cz[i]) / max(ARM_BODY_H, 0.01)
                        risk[i] += 5.0 + intrusion * 15.0
                    if z_overlap_ji and y_overlap:
                        # Grasping rear j: arm body would hit front i
                        # This is WORSE — arm passes through front object
                        arm_blocked[j] = True
        ...
        return risk, blocked, y_nbrs, arm_blocked, z_top, edge_x

    def _sort_objects(self, objects):
        """Collision-aware picking order.

        1. Layer (Z): top objects first (prevent stack collapse)
        2. Risk: lowest collision risk first (isolated & unblocked)
        3. X distance: closest to robot first (clear approach path)
        """
        n = len(objects.name)
        if n == 0:
            return []
        ...
        # Run risk analysis
        risk, blocked, y_nbrs, arm_blocked, z_arr, x_arr = \
            self._compute_collision_risk(objects, range(n))

        # Sort: -z (highest first), risk (safest first), x (closest first)
        scored = []
        for i in range(n):
            scored.append((-z_arr[i], risk[i], x_arr[i], i))
        scored.sort()
        sorted_indices = [s[3] for s in scored]

        # Log rationale
        n_types = len(objects.type)
        rospy.loginfo("Picking order (collision-aware, arm_body>=5cm):")
        rospy.loginfo("  %4s %-8s %6s %6s %6s %6s %6s %6s %s",
                      "Rank", "Name", "Z", "Xedge", "Risk", "XBlk",
                      "ArmBlk", "YNbrs", "Type")
        for rank, idx in enumerate(sorted_indices):
            obj_type = objects.type[idx] if idx < n_types else "??"
            rospy.loginfo("  %4d %-8s %6.3f %6.3f %6.1f %6s %6s %6d %s",
                          rank + 1, objects.name[idx],
                          z_arr[idx], x_arr[idx], risk[idx],
    ...
    def _get_approach_position(self, table_x, table_y, for_place=False):
        ...
        Source table (x < 0): robot approaches from +x direction, faces -x.
        Dest table (x > 0): robot approaches from -x direction, faces +x.

        When for_place is True, accounts for place_action's built-in forward
        movement (fPlaceX - 0.65 m) so the robot ends up at a safe distance
        from the table instead of crashing into it.
        Uses place_approach_offset for placing vs approach_offset for grabbing.
        """
        if table_x < 0:
            approach_x = table_x + self.table_half_depth + self.approach_offset
            yaw = math.pi   # face toward -x (left)
        else:
            if for_place:
                place_forward = table_x - 0.65  # place_action's total forward move
                approach_x = table_x - self.table_half_depth - self.place_approach_offset - place_forward
            else:
                approach_x = table_x - self.table_half_depth - self.approach_offset
            yaw = 0.0       # face toward +x (right)
        return approach_x, table_y, yaw
    ...
    def run(self):
        ...
        # Detect objects on source table
        if not self.detect_objects():
            rospy.logerr("No objects detected. Aborting.")
            self.state = 'DONE'
            self._publish_stats()
            return

        # Smart sorting: top-first (anti-collapse), then nearest-first
        sorted_indices = self._sort_objects(self.latest_objects)
        n_objects = len(sorted_indices)
        rospy.loginfo("Starting palletizing: %d objects (sorted: top-down, near-first)", n_objects)
        self._speak("ma duo ren wu kai shi, gong {} ge wu ti".format(n_objects))

        for order, idx in enumerate(sorted_indices):
            self.last_cycle_start = time.time()
            obj_name = self.latest_objects.name[idx]
            obj_type = (self.latest_objects.type[idx]
                        if idx < len(self.latest_objects.type) else "")

            # Route to correct zone based on vision classification
            zone = self._get_zone(obj_type)
            zone_material = 'hard' if 'hard' in obj_type else 'soft'
            obj_height = self._get_object_height(obj_type)

            rospy.loginfo("--- Object %d/%d: %s (type: %s, zone: %s, height: %.2f) ---",
                          order + 1, n_objects, obj_name, obj_type, zone_material, obj_height)

            self._speak("di {} ge, {} qu".format(order + 1, zone_material))

            # For objects after the first, navigate back to source table
            if order > 0:
                rospy.loginfo("Returning to source table for next object...")
                if not self.navigate_to_table(self.source_table_x, self.source_table_y):
                    rospy.logerr("Failed to return to source table. Aborting remaining.")
                    self.objects_failed += n_objects - order
                    break

            # Step 1: Grab at this specific object's center (Y is already center
            # from objects_3d; X is xMin → convert to center; Z is zMax → center)
            obj_x_edge = self.latest_objects.x[idx] if idx < len(self.latest_objects.x) else 0.0
            obj_y = self.latest_objects.y[idx] if idx < len(self.latest_objects.y) else 0.0
            obj_z_top = self.latest_objects.z[idx] if idx < len(self.latest_objects.z) else 0.0
            # Estimate object half-width from height (cubes are roughly square)
            obj_half = obj_height / 2.0
            obj_center_x = obj_x_edge + obj_half
            obj_center_z = obj_z_top - obj_half
            rospy.loginfo("Target object: edge_x=%.2f → center=(%.2f, %.2f, %.2f) h=%.2f",
                          obj_x_edge, obj_center_x, obj_y, obj_center_z, obj_height)
            if not self.grab_object(obj_type, obj_center_x, obj_y, obj_center_z, timeout=90.0):
            ...
            if not self.navigate_to_table(self.dest_table_x, self.dest_table_y, for_place=True):
                ...
                release_cmd.position = [0.5, 0.15]  # safe height, open gripper
                for _ in range(10):
        ...
        rospy.loginfo("Palletizing complete! %d/%d succeeded (%.1f%%).",
                      self.objects_succeeded, n_objects,
                      100.0 * self.objects_succeeded / max(n_objects, 1))
