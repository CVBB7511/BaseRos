...
class CargoFeatureCaptureNode:
    def __init__(self):
        ...
        self.rgb_topic = rospy.get_param("~rgb_topic", defaults.get("rgb_topic", "/kinect2/qhd/image_color_rect"))
        self.depth_topic = rospy.get_param("~depth_topic", defaults.get("depth_topic", "/kinect2/qhd/image_depth_rect"))
        self.camera_info_topic = rospy.get_param(
            "~camera_info_topic",
            defaults.get("camera_info_topic", "/kinect2/qhd/camera_info"),
        )
        self.roi = rospy.get_param("~roi", defaults.get("roi", {"x": 0.35, "y": 0.30, "width": 0.30, "height": 0.35}))
        self.hsv_padding = rospy.get_param("~hsv_padding", defaults.get("hsv_padding", {"h": 8, "s": 35, "v": 35}))
        self.min_area = int(rospy.get_param("~min_area", defaults.get("min_area", 1200)))
