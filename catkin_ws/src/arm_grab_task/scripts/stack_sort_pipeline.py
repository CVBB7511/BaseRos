...
class FieldGeometry:
    def __init__(self):
        ...
        self.table_height = self._float(raw.get("table_height"), 0.78)
...
class RunMetrics:
    def __init__(self, colors=None):
        colors = [str(c) for c in (colors or ["red", "green", "blue"])]
...
class MotionArmController:
    ...
    def move_distance(self, distance: float, dist_tolerance: Optional[float] = None):
        if abs(distance) < 1e-3:
            return True, 0.0

        if (not self.use_odom_control) or (not self.odom_ready()):
...
class StackSortOrchestrator:
    ...
    def _pick_sequence(self) -> bool:
        ...
        self.ctrl.move_lift_and_gripper(
            lift=self.pick_lift_height,
            gripper=self.open_gripper,
            duration=self.arm_open_seconds,
        )

        if self.pick_insert_distance > 0.0:
            rospy.loginfo(
                "[PICK] insert forward %.3f tolerance=%.3f"
                % (self.pick_insert_distance, self.pick_insert_tolerance)
            )
            self._forward_by(self.pick_insert_distance, dist_tolerance=self.pick_insert_tolerance)
    ...
    def _drop_sequence(self, color: str):
        if self.tabletop_drop_mode:
            self._tabletop_drop_sequence(color)
            return

        yaw, forward, drop_lift = self.planner.get_drop_plan(color)
        rospy.loginfo("[DROP] move to zone color=%s" % color)
        self._rotate_by(yaw)
        self._forward_by(forward)

        self._visual_refine_drop_alignment(color)
        self._correct_drop_pose_to_anchor(color)

        rospy.loginfo("[DROP] place at lift %.3f" % drop_lift)
        self.ctrl.set_lift_and_gripper(lift=drop_lift, gripper=self.closed_gripper)
        rospy.sleep(1.0)
        self.ctrl.set_lift_and_gripper(lift=drop_lift, gripper=self.open_gripper)
        rospy.sleep(0.8)
        self.ctrl.set_lift_and_gripper(lift=self.carry_lift_height, gripper=self.open_gripper)
        rospy.sleep(0.6)
    ...
    def _tabletop_drop_sequence(self, color: str):
        ...
        rospy.loginfo(
            "[DROP] tabletop place color=%s layer=%d yaw=%.3f forward=%.3f lift=%.3f settle=%.2f"
            % (
                color,
        ...
        if self.tabletop_use_base_targets and base_target is not None:
            self._drive_to_pose(base_target, "drop_base_%s" % color)
        else:
            self._rotate_by(yaw)
            self._forward_by(forward)
        self.ctrl.move_lift_and_gripper(
        ...
        if self.tabletop_retreat_after_place > 0.0:
            self._forward_by(-self.tabletop_retreat_after_place)
        if self.tabletop_use_base_targets and self.tabletop_return_to_pick_pose and pick_pose is not None:
            ...
            self._drive_to_pose(return_target, "return_pick_%s" % color)
        else:
            self._forward_by(-forward)
            self._rotate_by(-yaw)
        ...
        while not rospy.is_shutdown():
            ...
            if self.state == "SEARCH":
                ...
                if color is None:
                    ...
                    if self._drive_to_source_pick_target(color):
                ...
                if (not grasp_ok) or self._pick_looks_failed():
