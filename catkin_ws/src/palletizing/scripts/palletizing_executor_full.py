...
class PalletizingExecutor:
    ...
    def __init__(self):
        ...
        self.detect_retry_count = rospy.get_param('~detect_retry_count', 3)
        self.detect_retry_settle = rospy.get_param('~detect_retry_settle', 0.30)
    ...
    def _get_robot_position(self):
        ...
        quaternion. Raises RuntimeError when TF is unavailable, because a fake
        fallback pose would corrupt downstream map/base conversions.
        """
        try:
    ...
    @staticmethod
    def _angle_diff(a, b):
        """Return signed shortest angular distance a-b in radians."""
        return math.atan2(math.sin(a - b), math.cos(a - b))

    def _map_point_to_base_tf(self, map_x, map_y, map_z=0.0):
        """Transform a map-frame point to base_link using TF."""
        point = PointStamped()
        ...
        try:
            self.tf_listener.waitForTransform(
                '/base_link', '/map', rospy.Time(0), rospy.Duration(0.5))
            base_point = self.tf_listener.transformPoint('/base_link', point)
            return base_point.point.x, base_point.point.y, base_point.point.z
        except (tf.LookupException, tf.ConnectivityException,
                tf.ExtrapolationException, tf.Exception) as e:
            raise RuntimeError("TF transform /map->/base_link point failed: %s" % e)

    def _near_nav_goal(self, nav_x, nav_y, nav_yaw=None):
        ...
        yaw_err = 0.0 if nav_yaw is None else abs(self._angle_diff(robot_yaw, nav_yaw))
        yaw_ok = nav_yaw is None or yaw_err <= self.nav_accept_yaw_tolerance
        if dist <= self.nav_accept_xy_tolerance and yaw_ok:
            if nav_yaw is None:
                rospy.loginfo("Navigation within tolerance: dist=%.3fm <= %.3fm",
                              dist, self.nav_accept_xy_tolerance)
            else:
                rospy.loginfo("Navigation within tolerance: dist=%.3fm, yaw_err=%.1f°",
                              dist, math.degrees(yaw_err))
            return True
        rospy.logwarn("Navigation outside tolerance: dist=%.3fm/%.3fm, yaw_err=%.1f°/%.1f°",
                      dist, self.nav_accept_xy_tolerance,
                      math.degrees(yaw_err), math.degrees(self.nav_accept_yaw_tolerance))
        return False

    def _wait_robot_settled(self, duration=None):
        """Stop residual base motion and wait briefly before perception/control."""
        if duration is None:
    ...
    def _detect_with_retry(self, retries=None, settle=None):
        """Detect objects after settling; retry instead of using stale coords."""
        if retries is None:
            retries = self.detect_retry_count
        if settle is None:
            settle = self.detect_retry_settle
        for attempt in range(max(1, retries)):
            self._wait_robot_settled(settle)
            if self.detect_objects():
                return True
            rospy.logwarn("Object detection attempt %d/%d failed",
                          attempt + 1, max(1, retries))
        return False
    ...
    def grab_object(self, obj_type="", target_x=0.0, target_y=0.0, target_z=0.0,
        ...
        if self.grab_feedback == 'failed':
            rospy.logwarn("Grab failed in grab_action")
            return False
    ...
    def navigate_to_pose(self, nav_x, nav_y, nav_yaw, timeout=None):
        ...
        if state == actionlib.GoalStatus.SUCCEEDED:
            ...
            return True
        elif self._near_nav_goal(nav_x, nav_y, nav_yaw):
    ...
    def navigate_to_table(self, target_x, target_y, timeout=None, use_approach=True, for_place=False):
        ...
        if state == actionlib.GoalStatus.SUCCEEDED:
            rospy.loginfo("Navigation succeeded")
            return True
        elif self._near_nav_goal(nav_x, nav_y, yaw):
            rospy.logwarn("Navigation action state=%d, accepting near-goal pose", state)
            return True
        else:
            rospy.logwarn("Navigation failed (state: %d)", state)
    ...
    def run(self):
        ...
        while True:
            ...
            if not self._detect_with_retry():
            ...
            # First detection is used only to choose an alignment target.  The
            # actual grabbed object is selected again after navigation.
            ...
            rospy.loginfo("--- Alignment target %s (type: %s, zone: %s, h: %.2f, bf: %.3f,%.3f,%.3f) ---",
            ...
            try:
                ...
                rospy.logerr("Cannot align to source object: %s", e)
            ...
            side_x = -math.sin(align_yaw)
            side_y = math.cos(align_yaw)
            nav_x = det_rx + side_x * obj_bf_y
            nav_y = det_ry + side_y * obj_bf_y
            rospy.loginfo("Nav to source alignment (%.3f, %.3f, yaw=%.1f°, lateral=%.3f, current yaw=%.1f°)",
                          nav_x, nav_y, math.degrees(align_yaw),
                          obj_bf_y, math.degrees(det_yaw))
            if not self.navigate_to_pose(nav_x, nav_y, align_yaw):
            ...
            # ── Re-detect AFTER alignment.  Scheme A: select the current best
            # object from fresh local coordinates; do not match against stale
            # pre-navigation map coordinates.
            if not self._detect_with_retry():
                rospy.logwarn("Re-detect after alignment failed; returning to source approach without grabbing")
                self.objects_failed += 1
                self.objects_processed += 1
                self.cycle_times.append(time.time() - self.last_cycle_start)
                self._publish_stats()
            ...
            fresh_sorted = self._sort_objects(self.latest_objects)
            pick_idx = fresh_sorted[0]
            obj_name = self.latest_objects.name[pick_idx]
            obj_type = self._object_type(self.latest_objects, pick_idx)
            obj_height = self._get_object_height(obj_type)
            obj_half = obj_height / 2.0
            zone = self._get_zone(obj_type)
            zone_material = 'hard' if 'hard' in obj_type else 'soft'
            fresh_x = self.latest_objects.x[pick_idx] if pick_idx < len(self.latest_objects.x) else 0.0
            fresh_y = self.latest_objects.y[pick_idx] if pick_idx < len(self.latest_objects.y) else 0.0
            fresh_z = self.latest_objects.z[pick_idx] if pick_idx < len(self.latest_objects.z) else 0.0
            grab_x = fresh_x - obj_half
            grab_y = fresh_y
            grab_z = fresh_z - self.PRISM_Z_OFFSET + obj_half
            rospy.loginfo("Fresh pick idx=%d: %s type=%s zone=%s bf(%.3f, %.3f, %.3f) -> grab(%.3f, %.3f, %.3f)",
                          pick_idx, obj_name, obj_type, zone_material,
            ...
            try:
                ...
                place_x, place_y, _place_base_z = self._map_point_to_base_tf(
                    place_map_x, place_map_y, place_z)
            except RuntimeError as e:
                rospy.logerr("Cannot convert place pose without robot TF: %s", e)
                self.objects_failed += 1
                self.objects_processed += 1
                self.cycle_times.append(time.time() - self.last_cycle_start)
                self._publish_stats()
